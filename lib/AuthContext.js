...
export const AuthProvider = ({ children }) => {
    ...
    const updateProgress = async (newData) => {
        ...
        try {
            ...
            // 2. If subscription changed, sync to profiles and user_subscriptions for Admin Panel
            if (newData.subscription) {
                await supabase
                    .from('profiles')
                    .update({ plan_id: newData.subscription })
                    .eq('id', user.id);
                ...
                await supabase
                    ...
                    .upsert({
                        user_id: user.id,
                        plan_id: newData.subscription,
                        status: 'active',
                        updated_at: new Date().toISOString()
                    });
            }
        } catch (err) {
            console.error('Error updating progress/subscription:', err);
        }
    };
