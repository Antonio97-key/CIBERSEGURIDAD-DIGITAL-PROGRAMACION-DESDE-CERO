...
export default function LessonPage() {
    ...
    if (!module || !lesson || !curriculum.ciberseguridad[module]) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                <div className="w-12 h-12 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin"></div>
                <p className="text-gray-500 font-medium animate-pulse">Invocando el conocimiento digital...</p>
            </div>
        );
    }
    ...
    const prev = prevLesson ? { title: prevLesson.title, path: `/ciberseguridad/${module}/${prevLesson.id}` } : null;
    const next = nextLesson ? { title: nextLesson.title, path: `/ciberseguridad/${module}/${nextLesson.id}` } : null;
            ...
            return (
                <div key={index} className="space-y-6">
                    {part.split('\n').map((line, i) => {
                        ...
                        if (trimmed.startsWith('### ')) {
                            return <h3 key={i} className="text-xl font-bold mt-10 mb-4 text-blue-400">
                                {trimmed.replace('### ', '')}
                            </h3>;
                        }
                        if (trimmed.startsWith('- ') || trimmed.startsWith('• ')) {
                            return <li key={i} className="ml-6 text-gray-400 list-disc marker:text-blue-500 transition-colors hover:text-gray-300">{trimmed.substring(2)}</li>;
                        }
                        if (/^\d+\./.test(trimmed)) {
                            return <li key={i} className="ml-6 text-gray-400 list-decimal marker:text-blue-500 marker:font-black">{trimmed.replace(/^\d+\.\s*/, '')}</li>;
                        }
